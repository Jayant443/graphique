class Graph {
    ...
    async runShortestPath(startNodeId, endNodeId, onStep) {
        ...
        this.algoController = controller;

        const adj = this.getGraphData().adjList;
    ...
    async runDFS(startNodeId, onStep) {
        ...
        const dfs = async (node) => {
            ...
            const neighbors = this.getGraphData().adjList.get(node.id);
            for (const neighbor of neighbors) {
                if (controller.aborted) return;
                if (!visited.has(neighbor.id)) {
                    const neighborNode = this.nodes.get(neighbor.id);
    ...
    stopAlgorithm() {
        if (this.algoController) {
            this.algoController.aborted = true;
        }
        this.clearHighlights();
    }
